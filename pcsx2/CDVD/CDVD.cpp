...
{ // NCOMMAND
	...
	switch (rt)
	{
		...
		{
			// Assign the seek to sector based on cdvd.SCMDParam[0]-[3], and the number of  sectors based on cdvd.SCMDParam[4]-[7].
...
{
	{
		...
		switch (rt)
		{
			...
			case 0x03: // Mecacon-command
				switch (cdvd.SCMDParam[0])
				{
					...
					case 0x30:
						...
						//Console.Warning("Tray check param[1]=%02X", cdvd.SCMDParam[1]);
						break;
			...
			case 0x08: // CdReadRTC (0:8)
				...
				/*Console.WriteLn("RTC Read Sec %x Min %x Hr %x Day %x Month %x Year %x", cdvd.SCMDResult[1], cdvd.SCMDResult[2],
				  cdvd.SCMDResult[3], cdvd.SCMDResult[5], cdvd.SCMDResult[6], cdvd.SCMDResult[7]);
			...
			case 0x09: // sceCdWriteRTC (7:1)
				...
				/*Console.WriteLn("RTC write incomming Sec %x Min %x Hr %x Day %x Month %x Year %x", cdvd.SCMDParam[cdvd.SCMDParamP-7], cdvd.SCMDParam[cdvd.SCMDParamP-6],
				  cdvd.SCMDParam[cdvd.SCMDParamP-5], cdvd.SCMDParam[cdvd.SCMDParamP-3], cdvd.SCMDParam[cdvd.SCMDParamP-2], cdvd.SCMDParam[cdvd.SCMDParamP-1]);
				  Console.WriteLn("RTC Write Sec %d Min %d Hr %d Day %d Month %d Year %d", cdvd.RTC.second, cdvd.RTC.minute,
				  cdvd.RTC.hour, cdvd.RTC.day, cdvd.RTC.month, cdvd.RTC.year);*/
				//memcpy((u8*)&cdvd.RTC, cdvd.SCMDParam, 7);
				break;
			...
			case 0x32: //sceCdGetMediumRemoval (0:2)
				SetSCMDResultSize(2);
				cdvd.SCMDResult[0] = 0;
				//cdvd.SCMDResult[0] = 0; // fixme: I'm pretty sure that the same variable shouldn't be set twice here. Perhaps cdvd.SCMDResult[1]?
				break;
			...
			case 0x36: //cdvdman_call189 [__sceCdReadRegionParams - made up name] (0:15) i think it is 16, not 15
				...
				//////////////////////////////////////////
				cdvd.SCMDResult[2] = 0; //??
				//			cdvd.SCMDResult[3] == ROMVER[4] == *0xBFC7FF04
				//			cdvd.SCMDResult[4] == OSDVER[4] == CAP			Jjpn, Aeng, Eeng, Heng, Reng, Csch, Kkor?
				//			cdvd.SCMDResult[5] == OSDVER[5] == small
				//			cdvd.SCMDResult[6] == OSDVER[6] == small
				//			cdvd.SCMDResult[7] == OSDVER[7] == small
				//			cdvd.SCMDResult[8] == VERSTR[0x22] == *0xBFC7FF52
